<script lang="ts">
  ...
  let visibleStreamIdList = data.streamList.map((stream) => stream.id)

  $: zonedInstant = utcToZonedTime(instant, timeZone)
  $: title = dateFns.format(zonedInstant, 'PPPP')

  $: previousWeek = dateFns.format(
    dateFns.subWeeks(zonedInstant, 1),
    'yyyy-MM-dd',
  )
  $: nextWeek = dateFns.format(dateFns.addWeeks(zonedInstant, 1), 'yyyy-MM-dd')
...
{#each streamList as stream}
  <input
    type="checkbox"
    bind:group={visibleStreamIdList}
    value={stream.id}
    id={stream.id}
  />
