...
let effective_options = $derived(options ?? [])
...
let disabled_option_keys = $derived(
  new Set(
    effective_options.filter((opt) => is_object(opt) && opt.disabled).map(key),
  ),
)
...
$effect.pre(() => {
  ...
  matchingOptions = effective_options.filter((opt) => {
    ...
    const keep_in_list =
      !selected_keys_set.has(key(opt)) || duplicates || keepSelectedInDropdown
    if (!keep_in_list) {
      return false
    }

    // check search match
    return matches_search(opt, searchText)
  })
})
...
function add(input: Option, event: Event) {
  event.stopPropagation()
  if (maxSelect !== null && selected.length >= maxSelect) {
    wiggle = true
  }

  let option_to_add = input
  if (
    !Number.isNaN(Number(option_to_add)) &&
...
function handle_dropdown_after_select(event: Event) {
  const reached_max = selected.length >= (maxSelect ?? Number.POSITIVE_INFINITY)
  const should_close =
...
async function handle_arrow_navigation(direction: 1 | -1) {
  ...
  if (autoScroll) {
    await tick()
    document.querySelector('ul.options > li.active')?.scrollIntoView()
  }
}
...
function batch_add_options(options_to_add: Option[], event: Event) {
  const remaining = Math.max(
    0,
    (maxSelect ?? Number.POSITIVE_INFINITY) - selected.length,
  )
...
const drop = (target_idx: number) => (event: DragEvent) => {
  ...
  const start_idx = Number.parseInt(
...
const handle_input_keydown: KeyboardEventHandler<HTMLInputElement> = (
  ...
  void handle_keydown(event) // Restore internal logic
...
const handle_input_blur: FocusEventHandler<HTMLInputElement> = (event) => {
  ...
  if (!outerDiv?.contains(event.relatedTarget as Node)) {
    close_dropdown(event)
  }
...
$effect.pre(() => {
  required // trigger effect when required changes
  form_input?.setCustomValidity('')
})
